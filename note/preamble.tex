...
% Зачем: позволяет избежать переполнения строк
% в некоторых случаях.
\sloppy
...
\newcommand{\addappendix}[2][]{%
  \clearpage
  \refstepcounter{appendixcount}%
  \begin{center}%
    \bfseries % Делает жирным всё содержимое центра
    \MakeUppercase{Приложение~\Asbuk{appendixcount}}%
    \if\relax\detokenize{#1}\relax%
      % Если типа нет
      \\%
    \else%
      % Если тип есть, он теперь тоже будет жирным
      \\[0ex](#1)\\%
    \fi%
    #2%
  \end{center}%
  \addcontentsline{toc}{section}{Приложение~\Asbuk{appendixcount}\if\relax\detokenize{#1}\relax\else\ (#1)\fi\ #2}%
}
